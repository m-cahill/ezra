...
| M37B | Required Gate Recovery Implementation | In progress — Pending merge review | — | PR#39 | Implementation complete on branch `fix/m37b-required-gate-recovery`: advisory lockfile/`pip-audit`, `types-jsonschema`, Distribution Verification `ci-local` vs dispatch `release`, honest SLSA/Pages gating, Dependency Review documented as infra. Tip CI validated @ `237451b` run `25468913909` (see `M37B_run1.md` merge hygiene if head advances). Closeout docs: `M37B_run1.md`, `M37B_summary.md`, `M37B_audit.md`. **Do not mark milestone Complete in this row until after PR #39 merge** (project convention). M37 remains deferred until merge. |
