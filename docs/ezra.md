...
| M37B | Required Gate Recovery Implementation | In progress — Pending CI | — | — | Branch `fix/m37b-required-gate-recovery`: lockfile/`pip-audit`, Distribution Verification (ci-local vs dispatch release mode), conditional SLSA attestation + summaries, Pages deploy behind `EZRA_ENABLE_PAGES_DEPLOY`. M37 secret cleanup remains deferred. Closeout: `M37B_summary.md`, `M37B_audit.md` after CI. |
