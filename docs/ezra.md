...
| M37 | Public Release Boundary Cleanup | Implementation complete — pending merge review | — | PR#40 | Removed only three tracked `docs/enhancements/*.md`; `git ls-files .cursorrules docs/enhancements docs/prompts` is **empty**; guardrail `tests/test_public_release_boundary.py`. `.gitignore` covers `.cursorrules`, `docs/enhancements/`, `docs/prompts/`. No runtime, EPB, dependency, or workflow behavior changes. **Do not mark milestone Complete until after PR #40 merge.** Closeout: `M37_summary.md`, `M37_audit.md`, `M37_run1.md`. Dependency Review remains PR-only infra on this repo. |
